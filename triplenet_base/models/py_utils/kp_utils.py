...
def _gather_feat(feat, ind, mask=None):
    #FEAT : B HW 1/2， ind: B K
    dim  = feat.size(2)
    ind  = ind.unsqueeze(2).expand(ind.size(0), ind.size(1), dim)  #B K 1 -> B K 1/2
...
def _tranpose_and_gather_feat(feat, ind):
    #FEAT: B, 2, H, W,
    feat = feat.permute(0, 2, 3, 1).contiguous()  # B H W 2
    feat = feat.view(feat.size(0), -1, feat.size(3)) # B WH, 2
    feat = _gather_feat(feat, ind)
    return feat

def _topk(scores, K=20):
    batch, cat, height, width = scores.size() # B C H W  -> B CHW
            
    topk_scores, topk_inds = torch.topk(scores.view(batch, -1), K)  # B K '' B K

    topk_clses = (topk_inds / (height * width)).int()

    topk_inds = topk_inds % (height * width)   # 在图片flatten后hw中的第几个，
    topk_ys   = (topk_inds / width).int().float()  # B, K
...
def _decode(
    ...
    dists  = torch.abs(tl_tag - br_tag)  # B K K 
    ...
    scores, inds = torch.topk(scores, num_dets) # B, 1000
    scores = scores.unsqueeze(2)  # B, 1000, 1

    bboxes = bboxes.view(batch, -1, 4)  
    bboxes = _gather_feat(bboxes, inds)  #B, 1000, 4
    ...
    ct_xs = ct_xs[:,0,:]
    ct_ys = ct_ys[:,0,:]
    #ct 有k个 B, K  -> B, K, 1 detection有1000个
